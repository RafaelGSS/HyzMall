...
namespace wpp {
	namespace net {
		namespace http {
			enum http_response_code : unsigned int {
...
#pragma pack(pop)
			...
			public:
				...
				http_exception(http_response_code _code) {
					__code = _code;
					make_str_error();
				}
				...
				virtual http_response_code code() {
					return __code;
				}
			...
			public:
				...
				*/
				...
				virtual std::string post(
					...
					http_response_code _error;
					set_busy(true);
					std::string query_content = post(_url, content, _error, timeout);
					set_busy(false);
					if (_error != http_response_code::http_response_ok) {
				...
				virtual std::string do_request(
					...
					std::string retval;
					http_response_code _error = http_response_code::http_response_ok;
					set_busy(true);
					if (post) {
