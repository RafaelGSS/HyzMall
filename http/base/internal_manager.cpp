...
std::shared_ptr<std::string> internal_manager::send(internal_ptr r, uint32_t& ec, uint32_t max_tries)
{
	if (!max_tries)
	{
		return nullptr;
	...
	try
	{
		...
		auto res = std::make_shared<std::string>(client_curl.post(url, r->get_data()));
